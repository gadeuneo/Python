...
class BrickBreaker: 
    ...
    def draw(self, brickGroup):
        ...
        #Draw end text:
        if self.endText:
            self.win.blit(self.endText, (0, 0))
    ...
    def gameLoop(self, brickGroup, lastLevel = False):
        ...
        #Set font for end screen
        endFont = pygame.font.SysFont("tahoma", 22)
        ...
        if self.state == "Loss":
            ...
            self.endText = endFont.render("Game Over. Score: " + numBricksHit, 1, (0, 0, 0)) 

        #Display win message if won
        elif self.state == "Win":
            self.endText = endFont.render("You Win!", 1, (0, 0, 0))
            
        #End screen loop
        while not self.quit and (self.state == "Loss" or self.state == "Win"):
